...
function mountSomaScrollPreview(root, options = {}) {
  if (!root) return null;
  ...
  const renderer = new IntroRenderer(canvas, {
    useWoodColors: true,
    useWoodGrain: true,
    distance: options.distance || 6.8,
    rotationX: options.rotationX || 0.45,
    rotationY: options.rotationY || 0.68,
    scrollOrbit: true,
    scrollOrbitAmount: options.scrollOrbitAmount || 0.7
  });
  ...
  const scheduleRender = () => {
    if (rafId) return;
    rafId = window.requestAnimationFrame(renderNow);
  };
...
window.BlockerHomePreview = {
  mountSomaScrollPreview
};
